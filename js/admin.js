...
async function saveProduct() {
    ...
    try {
        ...
        if (newVideoFile) { try { var uploadedVideoUrl = await uploadToCOS(newVideoFile, "videos"); if (!uploadedVideoUrl) uploadedVideoUrl = await uploadVideoToSupabase(newVideoFile); if (uploadedVideoUrl) videoUrl = uploadedVideoUrl; } catch(e) { console.error('视频上传失败:', e); showToast('视频上传失败'); } }
        var body = { name: name, description: desc, category: category, price: priceStr, code: code || null, specification: specification || null, images: imageUrls.length > 0 ? imageUrls : null, image_url: imageUrls.length > 0 ? imageUrls[0] : null, video: videoUrl };
